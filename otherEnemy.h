...
class Paratrooper : public AerialEnemy {
    ...
    public: 
        ...
        if (!landed)
        {
            ...
            if (height<= 0.0f){ // ground level on ground
                landed = true;
    ...
    void attack() override{
        if (landed && innerEnemy){ // Enemy on ground and is soldier then start to attack
            innerEnemy->attack();
        }
    }
    //update hte movement 
    ...
    void takeDamage(int amount, int weaponType, int hitDirection) override{
        ...
        if (innerEnemy){ // inner enemy taking damage
            innerEnemy->takeDamage(amount, weaponType, hitDirection);
        }
        if (Hp <= 0) { // if hp less or equal to 0 then die 
            Hp = 0; 
            die(); 
        }
    }
...
class MummyWarrior : public UndeadEnemy {
    ...
    public:
        ...
        void takeDamage(int amount, int weaponType, int hitDirection) override
        {
            ...
            if (weaponType == weapon_Fire || weaponType == weapon_Explosion){  // Permanently killed
                Hp = 0;
                die();
            }
            else{ // Crumble — will resurrect
                isCrumbled          = true;
...
class Martian : public AlienEnemy {
    ...
    public:
        ...
        void switchToPhase2(){  // Martian drops to ground level and uses pistol
            currentPhase = 2;
            state = state_Patrolling;
        }
        ...
        void attack() override{
            ...
            if (currentPhase == 1 && beamCooldown <= 0.0f){ // Fire energy beam downward when above player
                beamCooldown = 1.5f;
                state        = state_Attacking;
            }
            else if (currentPhase == 2 && pistolCooldown <= 0.0f){ // Fire pistol horizontally
                pistolCooldown = 1.0f;
                state          = state_Attacking;
            }
        }
        ...
        void update(float TimeChange) override{
            ...
            if (beamCooldown   > 0.0f) beamCooldown   -= 0.016f;
            if (pistolCooldown > 0.0f) pistolCooldown -= 0.016f;
