...
class FlyingTara : public VehicleEnemy {
    ...
    public:
        ...
        //movement function
        void move(float TimeChange) override { // Move horizontally until above player then hover
            if (!isAbovePlayer){
...
class M15ABradley : public VehicleEnemy {
    ...
    public:
        ...
        void move(float TimeChange) override{
            if (!isStationary){ // Moving horizontally on flat surface toward player
                state = state_Chasing;
            }
        }
        Enemy* create(float x, float y, int biome) override{
...
class EnemySub : public VehicleEnemy {
    ...
    public:
        ...
        void move(float TimeChange) override{ // Patrol horizontally underwater
            state = state_Patrolling;
        }
        Enemy* create(float x, float y, int biome) override{
